...
- Add ``method="aumann_shapley"`` to ``mtq.auto_quantize``: label-free sensitivity scoring that also reports a ``predicted_damage`` estimate, in calibration-KL units, for the allocation it selects. Method-specific settings -- including a ``max_predicted_damage`` mode that bounds that estimate instead of the bit budget -- are passed through the new optional ``method_options`` argument (also available as ``auto_quantize.method_options`` in a recipe).
