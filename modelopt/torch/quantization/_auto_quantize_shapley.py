...
- ``num_path_nodes`` (default 2): quadrature nodes for the path integral.
...
class AutoQuantizeAumannShapleySearcher(AutoQuantizeGradientSearcher):
    ...
    def _estimate_auto_quantize_scores(self, is_param_grad_enabled):
        ...
        def set_all_hparams(recipe_of) -> None:
            """Set every configurable hparam via ``fn``."""
            for hparam in hparams:
                hparam.active = recipe_of(hparam)

        def score_estimate_forward(module, input, *args, **kwargs):
            """Forward that emits the path-shifted output and caches its diffs."""
            ...
            # Only a pass that can repopulate the cache may clear it: score modules nest, and
            # the outer module's no_grad replay below re-enters the inner module's forward.
        ...
        def backward_hook(module, grad_input, grad_output):
            """Accumulate the attribution for this module's cached diffs."""
        ...
        def setup_params_for_score_estimation(name, param, params_metadata, enable_grad=True):
            """Enable grads on a parameter and hook its accumulation."""
        ...
        def setup_module_for_score_estimation(module):
            """Patch a module's forward and register its backward hook."""
        ...
        def cleanup_params_after_score_estimation(name, param, params_metadata):
            """Restore a parameter's original grad state."""
        ...
        def score_step(model_, data):
            """Run one scoring step over a batch."""
    ...
    def initialize_candidate_stats(self):
        ...
        if tokens <= 0:
            ...
            # damage_model.
            # Same key shape as the normal path.
            self.damage_model = {
        ...
        for i, name in enumerate(names):
            ...
            # QuantRecipe.__lt__ pins no_quant last, so the corner is always a quantized format.
