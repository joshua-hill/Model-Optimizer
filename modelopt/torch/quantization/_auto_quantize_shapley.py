...
class AutoQuantizeAumannShapleySearcher(AutoQuantizeGradientSearcher):
    ...
    def initialize_candidate_stats(self):
        ...
        if tokens <= 0:
            ...
            # Record an invalidated model rather than leaving it unset: the quote is keyed off
            # damage_model, so returning bare would omit predicted_damage entirely and a
            # consumer would hit KeyError instead of the nan every other failure path reports.
            # Same key shape as the normal path so a consumer following the documented
            # contract (completeness, f_corner, ...) gets NaN/None rather than a KeyError.
            self.damage_model = {
        ...
        for i, name in enumerate(names):
            ...
            if corner.is_no_quant:
                # Not an assert: the invariant is enforced by QuantRecipe.__lt__ in another
                # module, and under ``python -O`` a stripped assert would surface as a bare
                # KeyError on the next line instead of naming the violated ordering.
                raise ValueError(
    ...
    def _attach_predicted_damage(self) -> None:
        ...
        if not valid:
            # An invalidated fit leaves c = 0.0, so the coverage link would quote exactly
            # 0.0 -- the most reassuring number possible, in the one case it is meaningless.
            # NaN cannot be mistaken for "no damage" by a consumer reading only the quote.
            predicted = float("nan")
    ...
    def _run_damage_bound_search(self, max_predicted_damage, verbose=False):
        ...
        # ``valid`` is written on the common path for both links, so an absent key means no
        # fit was built at all -- also uncertifiable.
